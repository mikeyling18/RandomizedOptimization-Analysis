...
/**
 * Edited by Mikey Ling 10/10/2018
 * Adapted from code created by Daniel Cai on 3/5/2017
 */
...
public class abalone_ga {
    ...
    private static Instance[] train_set = Arrays.copyOfRange(instances, 0, 1985);
    private static Instance[] test_set = Arrays.copyOfRange(instances, 1985, 2835);

    private static DataSet set = new DataSet(train_set);

    private static int inputLayer = 8, hiddenLayer=12, outputLayer = 1;
    ...
    public static void main(String[] args) {
        ...
        int[] iterations = {10,20,100,250,500,1000};

        int[] population = {10,20,50,75,100,125};
        int[] mate = {5,10,25,50,75,100};
        int[] mute = {2,5,10,15,20,30};
    ...
    private static Instance[] initializeInstances() {
        int num_features = 8;
        int num_samples = 2835;
        double[][][] attributes = new double[num_samples][][];


        try {
            BufferedReader br = new BufferedReader(new FileReader(new File("src/opt/mling7/abalone.csv")));
            ...
            for(int i = 0; i < attributes.length; i++) {
                ...
                attributes[i][0] = new double[num_features]; // 8 attributes
                attributes[i][1] = new double[1]; // 0 = Female, 1 = Male
